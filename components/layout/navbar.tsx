...
import { useGetAll } from "@/schema/services/home/all";
      ...
      <div className="flex flex-row justify-between items-center w-full h-full border-b-[2px] border-[#E0E2DE] md:border-b-0">
                              ...
                              {classList.map((prop: any, index: number) => (
                                <div key={index}>
                                  <Button
                                    ...
                                    className="justify-start text-base w-full"
                                  >
                                    <Link href={`/class${prop.id}`} className={clsx("text-sm",pathname === `/${locale}/class/${prop.id}` ? ' font-bold text-[#BE5C59]' : "text-[#282B27]")}>{prop.name}</Link>
                                  </Button>
        ...
        <NavigationMenu className="hidden lg:block">
          <NavigationMenuList className="flex gap-[40px] items-center">
              {menu.map((item: any) => (
                item.url !== "/class" 
                ? <Link key={item.url} href={item.url} className={clsx("text-base px-2 text-[#000F30] hover:underline", pathname === item.url && "font-bold text-[#BE5C59] hover:no-underline")}>
                    {item.name}
                  </Link>
                : <div key={item.url}>
                        ...
                        <div className="grid w-[432px] grid-cols-2 gap-5 p-4 bg-[#FDF6EB]">
                          {classList.map((prop: any) => (
                            <Link
                              href={`/class/${prop.id}`}
                              key={prop.id}
                              className={clsx("p-3 text-sm hover:bg-[#fffcf7] line-clamp-2 rounded-md text-[#000F30]", pathname === `/${locale}/class/${prop.id}` && "font-bold text-[#BE5C59]")}
                            >
                              {prop.name}
                            </Link>
                          ))}
